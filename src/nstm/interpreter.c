...
// The Not So Tiny Machine (NSTM) Simulator
...
StepResult run_modi(Instr instr) {
    int d = is[instr.o3];
    if (d == 0) return DIV_ZERO;
    is[instr.o1] = is[instr.o2] / d;
    pc++;
...
StepResult run_modf(Instr instr) {
    float d = fs[instr.o3];
    if (d == 0.0) return DIV_ZERO;
    fs[instr.o1] = fs[instr.o2] / d;
    pc++;
...
StepResult run_neqi(Instr instr) {
    is[instr.o1] = is[instr.o2] != is[instr.o3];
    pc++;
    return OKAY;
}

StepResult run_neqf(Instr instr) {
    is[instr.o1] = fs[instr.o2] != fs[instr.o3];
    pc++;
    return OKAY;
}

StepResult run_neqs(Instr instr) {
    char *l = get_string(st, is[instr.o2]);
    char *r = get_string(st, is[instr.o3]);
    is[instr.o1] = ( strcmp(l, r) != 0 );
    pc++;
...
StepResult run_ltei(Instr instr) {
    is[instr.o1] = is[instr.o2] <= is[instr.o3];
    pc++;
    return OKAY;
}

StepResult run_ltef(Instr instr) {
    is[instr.o1] = fs[instr.o2] <= fs[instr.o3];
    pc++;
    return OKAY;
}

StepResult run_ltes(Instr instr) {
    char *l = get_string(st, is[instr.o2]);
    char *r = get_string(st, is[instr.o3]);
    is[instr.o1] = ( strcmp(l, r) <= 0 );
    pc++;
...
StepResult run_gthi(Instr instr) {
    is[instr.o1] = is[instr.o2] > is[instr.o3];
    pc++;
    return OKAY;
}

StepResult run_gthf(Instr instr) {
    is[instr.o1] = fs[instr.o2] > fs[instr.o3];
    pc++;
    return OKAY;
}

StepResult run_gths(Instr instr) {
    char *l = get_string(st, is[instr.o2]);
    char *r = get_string(st, is[instr.o3]);
    is[instr.o1] = ( strcmp(l, r) > 0 );
    pc++;
...
StepResult run_gtei(Instr instr) {
    is[instr.o1] = is[instr.o2] >= is[instr.o3];
    pc++;
    return OKAY;
}

StepResult run_gtef(Instr instr) {
    is[instr.o1] = fs[instr.o2] >= fs[instr.o3];
    pc++;
    return OKAY;
}

StepResult run_gtes(Instr instr) {
    char *l = get_string(st, is[instr.o2]);
    char *r = get_string(st, is[instr.o3]);
    is[instr.o1] = ( strcmp(l, r) >= 0 );
    pc++;
...
StepResult write_float32(int x) {
    printf("%f\n", fs[x]);
    pc++;
    return OKAY;
}
...
StepResult step() {
    ...
    switch(instr.op) {
        ...
        case MODi: return run_modi(instr);
        case MODf: return run_modf(instr);

        case EQUi: return run_equi(instr);
        case EQUf: return run_equf(instr);
        case EQUs: return run_equs(instr);
        case NEQi: return run_neqi(instr);
        case NEQf: return run_neqf(instr);
        case NEQs: return run_neqs(instr);
        case LTHi: return run_lthi(instr);
        case LTHf: return run_lthf(instr);
        case LTHs: return run_lths(instr);
        case LTEi: return run_ltei(instr);
        case LTEf: return run_ltef(instr);
        case LTEs: return run_ltes(instr);
        case GTHi: return run_gthi(instr);
        case GTHf: return run_gthf(instr);
        case GTHs: return run_gths(instr);
        case GTEi: return run_gtei(instr);
        case GTEf: return run_gtef(instr);
        case GTEs: return run_gtes(instr);
